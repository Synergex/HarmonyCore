...
:setup
start /wait rsynd -rs -n -c TestSrv -p %2
rem add xfserver env var
start /wait powershell Start-Process powershell -WindowStyle hidden -verb runAs -ArgumentList \"reg add 'HKEY_LOCAL_MACHINE\SOFTWARE\Synergex\Synergy xfServer\TestSrv\Synrc' /v %4 /d %3\"
start /wait rsynd -q -c TestSrv -p %2
start /wait rsynd -rs -n -c TestSrv -p %2
goto done
